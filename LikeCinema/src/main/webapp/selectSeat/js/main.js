...
;(function(window) {
	...
	// select a seat on the seat plan : 좌석 배치도에서 좌석을 선택하다
	function selectSeat(planseat) {
		// 전체 div 반복하면서 .row__seat--selected 클래스 갯수 카운트 후
		// 5개면 return false
		...
		if(document.querySelectorAll('.row__seat--selected').length == $('#inner2').val()){
			alert('최대 ' + $('#inner2').val() + '자리 까지 선택 가능합니다.');
			return false;
		}
