...
<html lang="en" class="no-js">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="./selectSeat/css/normalize.css" />
		<link rel="stylesheet" type="text/css" href="./selectSeat/css/demo.css" />
		<link rel="stylesheet" type="text/css" href="./selectSeat/css/component.css" />
		
		<script src="selectSeat/js/modernizr-custom.js"></script>
		<script type="text/javascript" src="./script/jquery-3.6.3.js"></script>
		<script type="text/javascript" src="./script/selectSeat.js"></script>
	
		<%	//예매번호 구해오기
		ReserveBoardDAO dao = new ReserveBoardDAO();
		int reserveNum = dao.getReserveNum();
		System.out.println("reserveNum 예약번호: " + reserveNum);
		...
		MemberDTO memberDto = reservedao.getReserveUserInfo(id);
		
		%>
		
		<%
		String inner = request.getParameter("hidden");
		String MovieId = request.getParameter("Movie_Id");
		String MovieTitle = request.getParameter("Movie_Title");
		String ReserveLocation = request.getParameter("Reserve_Location");
		String ReserveDate = request.getParameter("Reserve_Date");
		String ReserveTime = request.getParameter("Reserve_Time");
		String ReserveAdult = request.getParameter("Reserve_Adult");
		String ReserveTeenager = request.getParameter("Reserve_Teenager");
		String ReservePreferential = request.getParameter("Reserve_Preferential");
		String ReservePayPrice = request.getParameter("Reserve_Pay_Price");
...
<body>
	<header class="header">
		<div class="codrops-links">
			<a class="codrops-icon codrops-icon--prev" href="./Reservation.re" title="Previous Demo"><span>Previous Demo</span></a>
<!-- 				<a class="codrops-icon codrops-icon--drop" href="http://tympanus.net/codrops/?p=25885" title="Back to the article"><span>Back to the Codrops article</span></a> -->
	...
	<h1 class="header__title">뒤로 가기</h1>
...
<div class="container">
	<div class="cube">
		...
		<div class="cube__side cube__side--back">
			<div class="screen">
				<div class="video">
					<video class="video-player" src="" preload="auto" poster="./img/ss.jpg">
<!-- 					<video class="video-player" src="" preload="auto" poster="./img/ss.jpg"> -->
...
<!-- 							</div> -->
				<div class="intro__side">
					<button class="action action--seats">좌석을 선택해주세요.</button>
				</div>
			</div>
		</div>
	</div>
...
<form action="./Reservation.re" method="post" onclick="return seltSeat()"> 
<div class="plan">
	<h3 class="plan__title">좌석을 선택해주세요</h3>
	...
	<!-- /rows -->
	<ul class="legend">
		<li class="legend__item legend__item--free">예약가능</li>
		<li class="legend__item legend__item--reserved">예약불가능</li>
		<li class="legend__item legend__item--selected">선택 좌석</li>
	</ul>
	<input type="text" id="Reserve_Seat" name="Reserve_Seat" value="" style="width: 300px; border: none;background: transparent;font-size: 16px;color: white; text-align:center;">
	
	<input type="button" id="pay" class="action action--buy" value="결제하기">
	

	<input type="hidden" id="inner2" name="name" value="<%=inner %>" style="width : 240px">
	<input type="hidden" id="Reserve_Adult" name="Reserve_Adult" value="<%=ReserveAdult %>">
	<input type="hidden" id="Reserve_Teenager" name="Reserve_Teenager" value="<%=ReserveTeenager %>">
	<input type="hidden" id="Reserve_Preferential" name="Reserve_Preferential" value="<%=ReservePreferential %>">
	<input type="hidden" class="ticket_price" id="Reserve_Pay_Price" name="Reserve_Pay_Price" value="<%=ReservePayPrice %>">
	</div><!-- /plan -->
		<button class="action action--lookaround action--disabled" arial-label="Unlook View"></button>
		</form>
		<script src="selectSeat/js/classie.js"></script>
		<script src="selectSeat/js/main.js"></script>
	</body>
		...
		$(document).ready(function () {
		  ...
		  document.getElementById("pay").addEventListener("click",function requestPay() {
				...
				IMP.request_pay({
					...
					name : "<%=MovieTitle%>", // 영화 이름
					amount : "<%=ReservePayPrice%>", // 금액
					...
					if (rsp.success) { // 결제 성공 시,
						...
						$.ajax({
				            ...
				            data: {
				                ...
				                time: "<%=ReserveTime%>",
				                adult: "<%=ReserveAdult%>",
				                teen: "<%=ReserveTeenager%>",
				                prefer: "<%=ReservePreferential%>",
								seatList : $('#Reserve_Seat').val()
				            },
